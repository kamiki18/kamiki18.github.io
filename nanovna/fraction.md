...
<dl>
  <dd> $b/a = 1/(d_0 + 1/(d_1 + 1/(d_2 + 1/(d_3 ...))))$ </dd>
</dl>  
を得たとする。これを途中で打ち切ると有理近似数列  $b_0/a_0, b_1/a_1, b_2/a_2, b_3/a_3 ...$ になる。
つまり  $b_0/a_0 = 1/d_0, b_1/a_1 = 1/(d_0 + 1/d_1), b_2/a_2 = 1/(d_0 + 1/(d_1 + 1/d_2))), ...$ もちろん  $a_0 < a_1 < a_2 ...$ である。
...
有理近似の数列は右にいくほど真値に近く、また真値の両側から交互に真値に近付いていく。
したがって以下のどちらかが成り立つ。

<dl>
  <dd>  $ b_{m-1}/a_{m-1} <  b_{m+1}/a_{m+1} < b_m/a_m,  $ </dd>
  <dd>  $ b_m/a_m < b_{m+1}/a_{m+1} < b_{m-1}/a_{m-1} $  </dd>
</dl>

$b_{m-1}/a_{m-1}$ と $b_m/a_m$ の間の分数を $(b_{m-1} + k b_m)/(a_{m-1} + k a_m)$ と表現して、$a_{m-1} + k a_m < M$ の範囲で
この分数と $b_m/a_m$ のうち、優秀なほうを答えとする。
...
いや、こういうライブラリは常識で考えて、無限多倍長計算するものだろう。64bit たらずで破綻しないでほしい。
